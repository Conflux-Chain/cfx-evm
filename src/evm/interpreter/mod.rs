...
use crate::{
    ...
    vm::{
        ...
        GasLeft, MessageCallResult, ParamsType, ReturnData, Spec, TrapError, TrapKind,
    },
};
use bit_set::BitSet;
    ...
    fn exec(
        ...
        loop {
            ...
            match result {
                ...
                InterpreterResult::Done(value) => return vm::TrapResult::Return(value),
                InterpreterResult::Trap(trap) => match trap {
                    TrapKind::Call(params) => {
                        return vm::TrapResult::SubCallCreate(TrapError::Call(params, self));
                    }
        ...
        let result = match self.resume_result.take() {
            ...
            None => {
                ...
                let requirements = match self
                    ...
                    .as_mut()
                    .expect(GASOMETER_PROOF)
                    .requirements(context, instruction, info, &self.stack, self.mem.size())
                {
                    Ok(t) => t,
    ...
    fn exec_instruction(
        ...
        match instruction {
            ...
            instructions::CREATE | instructions::CREATE2 => {
                ...
                let can_create = context.balance(&self.params.address)? >= endowment
            ...
            | instructions::STATICCALL => {
                assert!(
                    context.spec().call_value_transfer_gas > context.spec().call_stipend,
                    "overflow possible"
                );
            ...
            instructions::SUICIDE => {
                ...
                context.suicide(&refund_address, tracer, context.spec().account_start_nonce)?;
                return Ok(InstructionResult::StopExecution);
            }
            instructions::LOG0
            | instructions::LOG1
            ...
            instructions::EXTCODEHASH => {
                let address = u256_to_address(&self.stack.pop_back());
                let hash = context.extcodehash(&address)?.unwrap_or_else(H256::zero);
...
mod tests {
    ...
    fn interpreter(params: ActionParams, context: &dyn vm::Context) -> Box<dyn Exec> {
        Factory::new(VMType::Interpreter, 1).create(params, context.spec(), context.depth())
    }
