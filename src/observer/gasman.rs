...
impl ExecutiveLevel {
    ...
    fn minimum_init_gas(&self, gas_left: &U256, ratio: (u64, u64)) -> U256 {
        ...
        self.gas_cost_this_level(gas_left)
            + (self.gas_limit_for_subcall * numerator + denominator - 1) / denominator
    }
}
...
impl GasMan {
    ...
    fn record_return(&mut self, gas_left: &U256) {
        ...
        }) = self.gas_record.last_mut()
        {
            ...
            *gas_limit_for_subcall += child_level.minimum_init_gas(gas_left, ratio);
        } else {
...
impl VmObserve for GasMan {
    fn record_call(&mut self, params: &ActionParams) {
        let cross_space_internal = params.code_address == *CROSS_SPACE_CONTRACT_ADDRESS;
