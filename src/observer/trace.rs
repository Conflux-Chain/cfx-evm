    ...
    fn decode(rlp: &Rlp) -> Result<Self, DecoderError> {
        rlp.as_val().and_then(|v| {
            Ok(match v {
                ...
                2 => Outcome::Fail,
                _ => {
                    return Err(DecoderError::Custom("Invalid value of CallType item"));
                }
            })
        })
    }
    ...
    fn from(r: &vmResult<FrameReturn>) -> Self {
        match r {
            Ok(FrameReturn {
                ...
                outcome: Outcome::Success,
                addr: create_address
                    .expect("Address should not be none in executive result of create"),
                gas_left: gas_left.clone(),
                return_data: return_data.to_vec(),
            },
            Ok(FrameReturn {
...
impl Action {
    ...
    pub fn bloom(&self) -> Bloom {
        match *self {
            ...
            Action::InternalTransferAction(ref internal_action) => internal_action.bloom(),
        }
    }
}
...
impl TransactionExecTraces {
    ...
    pub fn filter_trace_pairs(
        ...
        let mut trace_pairs: Vec<(ExecTrace, Option<ExecTrace>, usize)> = Vec::new();
            ...
            match &trace.action {
                ...
                Action::CallResult(_) | Action::CreateResult(_) => {
                        ...
                        .ok_or("result left unmatched!".to_string())?
                    {
                        ...
                        let subtraces = sublen_stack.pop().expect("stack_index matches");
    ...
    /// `from_address`, `to_address`, `action_types`, and `space` in `filter`
    /// are applied.
    pub fn filter_traces(self, filter: &TraceFilter) -> Result<Vec<ExecTrace>, String> {
        ...
        for trace in self.0 {
            match &trace.action {
                ...
                Action::CallResult(_) | Action::CreateResult(_) => {
                    if stack.pop().ok_or("result left unmatched!".to_string())? {
                        // Since we know that traces should be paired correctly,
                        // we do not check if the type
                        // is correct here.
                        traces.push(trace);
...
#[derive(Debug, PartialEq, Clone, Default, RlpEncodable, RlpDecodable, MallocSizeOf)]
