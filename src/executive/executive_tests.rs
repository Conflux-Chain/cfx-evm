...
use crate::{
    ...
    vm::{self, ActionParams, ActionValue, CallType, CreateContractAddress, Env, Spec},
...
use cfx_storage::{
    state_manager::StateManagerTrait, tests::new_state_manager_for_unit_test, StateIndex,
};
use cfx_types::{address_util::AddressUtil, Address, AddressSpaceUtil, BigEndianHash, U256, U512};
use keylib::{Generator, Random};
use primitives::{
    storage::STORAGE_LAYOUT_REGULAR_V0, transaction::Action, EpochId, NativeTransaction,
    Transaction,
};
...
fn make_byzantium_machine(max_depth: usize) -> Machine {
    let mut machine =
        crate::machine::new_machine_with_builtin(Default::default(), VmFactory::new(1024 * 32));
    machine.set_spec_creation_rules(Box::new(move |s, _| s.max_depth = max_depth));
    machine
}
...
fn test_contract_address() {
    ...
    let expected_address = Address::from_str("87ed868bd4e05f0be585961a5293a68cfb6ce60e").unwrap();
    assert_eq!(
        expected_address,
        contract_address(
...
fn test_sender_balance() {
    let sender = Address::from_str("1f572e5295c57f15886f9b263e2f6d2d6c7b5ec6").unwrap();
...
fn test_create_contract_out_of_depth() {
    ...
    let code =
        "7c601080600c6000396000f3006000355415600957005b60203560003555600052601d60036017f0600055"
            .from_hex()
            .unwrap();

    let sender = Address::from_str("1d1722f3947def4cf144679da39c4c32bdc35681").unwrap();
    let sender_with_space = sender.with_native_space();
    let address = contract_address(
        CreateContractAddress::FromSenderNonceAndCodeHash,
        /* block_number = */ 0.into(),
...
fn test_suicide_when_creation() {
    ...
    let sender_addr = Address::from_str("1d1722f3947def4cf144679da39c4c32bdc35681").unwrap();
    ...
    assert!(
        substate.storage_collateralized.get(&sender_addr).is_none(),
        "Since the contract has not been created, the sender occupied no storage now. "
    );
    assert!(
        substate.storage_released.get(&sender_addr).is_none(),
...
#[test]
fn test_revert() {
    let contract_address = Address::from_str("8d1722f3947def4cf144679da39c4c32bdc35681").unwrap();
...
fn test_deposit_withdraw_lock() {
    ...
    params.data = Some(
        "b6b55f250000000000000000000000000000000000000000000000000de0b6b3a7640000"
            .from_hex()
            .unwrap(),
    );
    ...
    params.call_type = CallType::Call;
    params.data = Some(
        "b6b55f250000000000000000000000000000000000000000000000000de0b6b3a763ffff"
            .from_hex()
            .unwrap(),
    );
    let mut tracer = ();
    ...
    // less data
    params.data = Some(
        "b6b55f25000000000000000000000000000000000000000000000000000000174876e8"
            .from_hex()
            .unwrap(),
    );
    let mut tracer = ();
    ...
    assert_eq!(
        result.unwrap_err(),
        vm::Error::InternalContract("ABI decode error: Incomplete static input parameter".into())
    );
    assert_eq!(
        state.balance(&sender_with_space).unwrap(),
        U256::from(1_000_000_000_000_000_000u64)
    ...
    // withdraw
    params.data = Some(
        "2e1a7d4d0000000000000000000000000000000000000000000000000000000ba43b7400"
            .from_hex()
            .unwrap(),
    );
    let mut tracer = ();
    ...
    params.data = Some(
        "2e1a7d4d0000000000000000000000000000000000000000000000000de0b6a803288c01"
            .from_hex()
            .unwrap(),
    );
    let mut tracer = ();
    ...
    assert_eq!(
        result.unwrap_err(),
        vm::Error::InternalContract("not enough withdrawable staking balance to withdraw".into())
    );
    assert_eq!(
        state.balance(&sender_with_space).unwrap(),
        U256::from(1_000_000_050_000_000_000u64)
    ...
    params.data = Some(
        "2e1a7d4d0000000000000000000000000000000000000000000000000de0b6a803288bff"
            .from_hex()
            .unwrap(),
    );
    let mut tracer = ();
    ...
    params.data = Some(
        "2e1a7d4d0000000000000000000000000000000000000000000000000de0b6a803288bfe"
            .from_hex()
            .unwrap(),
    );
    let mut tracer = ();
...
fn test_commission_privilege_all_whitelisted_across_epochs() {
    ...
    let mut state = State::new(StateDb::new(
        storage_manager
            .get_state_for_next_epoch(StateIndex::new_for_test_only_delta_mpt(&epoch_id))
            .unwrap()
            .unwrap(),
    ))
...
fn test_commission_privilege() {
    ...
    let options = TransactOptions::exec_with_no_tracing();
    let Executed { gas_used, .. } = Executive::new(&mut state, &env, &machine, &spec)
        .transact(&tx, options)
        .unwrap()
...
fn test_storage_commission_privilege() {
    ...
    state.checkpoint();
    state
        .remove_commission_privilege(address.address, sender.address(), caller1.address())
        .unwrap();
    let mut substate = Substate::new();
    assert_eq!(
        state
