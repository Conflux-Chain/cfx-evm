...
use super::super::{
    components::InternalRefContext, contracts::params_control::*, impls::staking::get_vote_power,
};
...
pub fn cast_vote(
    ...
    // the first version.
    let current_voting_version = (context.env.number - context.spec.cip94_activation_block_number)
    ...
    let old_version = context.storage_at(params, &storage_key::versions(&address))?;
    ...
    for vote in votes {
        ...
        match entry {
            ...
            Some(_) => {
                internal_bail!("Parameter voted twice: vote.index={}", vote.index);
            }
        }
    }
    if is_new_vote {
    ...
    for index in 0..PARAMETER_INDEX_MAX {
        ...
        for opt_index in 0..OPTION_INDEX_MAX {
            ...
            context.set_storage(params, vote_slot.to_vec(), param_vote[opt_index])?;
        }
        if !is_new_vote {
            RevokeEvent::log(
...
pub fn cast_vote_gas(length: usize, spec: &Spec) -> usize {
    let version_gas = 2 * spec.sload_gas + spec.sha3_gas + spec.sstore_reset_gas;

    let io_gas_per_topic = 3 * (2 * spec.sload_gas + 2 * spec.sstore_reset_gas + 2 * spec.sha3_gas);

    let log_gas_per_topic =
        2 * spec.log_gas + 8 * spec.log_topic_gas + 32 * 3 * 2 * spec.log_data_gas;
...
pub fn read_vote(
    ...
    for index in 0..PARAMETER_INDEX_MAX {
        ...
        if !deprecated_vote {
            for opt_index in 0..OPTION_INDEX_MAX {
                let votes =
                    context.storage_at(params, &storage_key::votes(&address, index, opt_index))?;
...
pub fn total_votes(version: u64, context: &mut InternalRefContext) -> vm::Result<Vec<Vote>> {
    let current_voting_version = (context.env.number - context.spec.cip94_activation_block_number)
        / context.spec.params_dao_vote_period
        + 1;
    ...
    for x in 0..PARAMETER_INDEX_MAX {
        ...
        answer.push(Vote {
            index: x as u16,
            votes: [
                state.get_system_storage(slot_entry[OPTION_UNCHANGE_INDEX as usize].as_ref())?,
                state.get_system_storage(slot_entry[OPTION_INCREASE_INDEX as usize].as_ref())?,
                state.get_system_storage(slot_entry[OPTION_DECREASE_INDEX as usize].as_ref())?,
            ],
        });
    }
...
lazy_static! {
    static ref CURRENT_VOTES_ENTRIES: [[[u8; 32]; OPTION_INDEX_MAX]; PARAMETER_INDEX_MAX] = {
        let mut answer: [[[u8; 32]; OPTION_INDEX_MAX]; PARAMETER_INDEX_MAX] = Default::default();
        for index in 0..PARAMETER_INDEX_MAX {
            for opt_index in 0..OPTION_INDEX_MAX {
                answer[index][opt_index] = system_storage_key::current_votes(index, opt_index);
            }
        }
        answer
    };
    static ref SETTLED_VOTES_ENTRIES: [[[u8; 32]; OPTION_INDEX_MAX]; PARAMETER_INDEX_MAX] = {
        ...
        for index in 0..PARAMETER_INDEX_MAX {
            for opt_index in 0..OPTION_INDEX_MAX {
                answer[index][opt_index] = system_storage_key::settled_votes(index, opt_index);
            }
        }
        answer
    };
...
impl ParamVoteCount {
    ...
    pub fn from_state<T: StateOpsTrait, U: AsRef<[u8]>>(
        ...
        Ok(ParamVoteCount {
            unchange: state
                .get_system_storage(slot_entry[OPTION_UNCHANGE_INDEX as usize].as_ref())?,
            increase: state
                .get_system_storage(&slot_entry[OPTION_INCREASE_INDEX as usize].as_ref())?,
            decrease: state
                .get_system_storage(&slot_entry[OPTION_DECREASE_INDEX as usize].as_ref())?,
        })
    }

    pub fn compute_next_params(&self, old_value: U256, pos_staking_tokens: U256) -> U256 {
...
pub fn get_settled_param_vote_count<T: StateOpsTrait>(state: &T) -> DbResult<AllParamsVoteCount> {
...
pub fn get_settled_pos_staking_for_votes<T: StateOpsTrait>(state: &T) -> DbResult<U256> {
...
pub fn settle_current_votes<T: StateOpsTrait>(
    state: &mut T,
    set_pos_staking: bool,
) -> DbResult<()> {
    for index in 0..PARAMETER_INDEX_MAX {
        for opt_index in 0..OPTION_INDEX_MAX {
            let vote_count = state.get_system_storage(&CURRENT_VOTES_ENTRIES[index][opt_index])?;
            state
                .set_system_storage(SETTLED_VOTES_ENTRIES[index][opt_index].to_vec(), vote_count)?;
    ...
    if set_pos_staking {
        let pos_staking = state.get_system_storage(&current_pos_staking_for_votes())?;
        state.set_system_storage(settled_pos_staking_for_votes().to_vec(), pos_staking)?;
        state.set_system_storage(
...
mod storage_key {
    ...
    pub fn votes(address: &Address, index: usize, opt_index: usize) -> [u8; 32] {
...
mod system_storage_key {
    ...
    pub(super) fn current_votes(index: usize, opt_index: usize) -> [u8; 32] {
        ...
        let base = base_slot(*PARAMS_CONTROL_CONTRACT_ADDRESS) + U256::from(CURRENT_VOTES_SLOT);
    ...
    pub(super) fn settled_votes(index: usize, opt_index: usize) -> [u8; 32] {
        ...
        let base = base_slot(*PARAMS_CONTROL_CONTRACT_ADDRESS) + U256::from(SETTLED_VOTES_SLOT);
    ...
    pub(super) fn current_pos_staking_for_votes() -> [u8; 32] {
        ...
        let base =
            base_slot(*PARAMS_CONTROL_CONTRACT_ADDRESS) + U256::from(CURRENT_POS_STAKING_SLOT);
        u256_to_array(base)
    }
    ...
    pub(super) fn settled_pos_staking_for_votes() -> [u8; 32] {
        // Position of `current_pos_staking` (static slot)
        let base =
            base_slot(*PARAMS_CONTROL_CONTRACT_ADDRESS) + U256::from(SETTLED_POS_STAKING_SLOT);
        u256_to_array(base)
    }
}
