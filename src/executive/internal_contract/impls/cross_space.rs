use crate::{
    ...
    state::cleanup_mode,
    vm::{
        self, ActionValue, CreateType, Exec, ExecTrapError as ExecTrap, ExecTrapResult, ParamsType,
        ResumeCall, ResumeCreate, Spec, TrapResult,
    },
};

use cfx_parameters::{
    block::CROSS_SPACE_GAS_RATIO, internal_contract_addresses::CROSS_SPACE_CONTRACT_ADDRESS,
};
use cfx_statedb::Result as DbResult;
use cfx_types::{Address, AddressSpaceUtil, AddressWithSpace, Bloom, Space, H256, U256};
use keccak_hash::keccak;
use primitives::{
    Action, Eip155Transaction, LogEntry, Receipt, SignedTransaction, TransactionOutcome,
};
...
use super::super::{
    ...
    contracts::cross_space::{CallEvent, CreateEvent, ReturnEvent, WithdrawEvent},
};
...
pub fn create_gas(context: &InternalRefContext, code: &[u8]) -> DbResult<U256> {
    ...
    let transaction_gas =
        gas_required_for(/* is_create */ true, code, context.spec) + context.spec.tx_gas as u64;
    ...
    let create_log_gas = {
        let log_data_length = H256::len_bytes() * 4 + (code_length + 31) / 32 * 32;
    ...
    Ok(create_gas + transaction_gas + address_mapping_gas + create_log_gas + return_log_gas)
}
...
pub fn call_gas(
    ...
    let transaction_gas =
        gas_required_for(/* is_create */ false, data, context.spec) + context.spec.tx_gas as u64;
    ...
    let call_gas = U256::from(context.spec.call_gas) + new_account_gas + transfer_gas;

    let address_mapping_gas = context.spec.sha3_gas * 2;

    let call_log_gas = {
        let log_data_length = H256::len_bytes() * 4 + (data_length + 31) / 32 * 32;
        context.spec.log_gas
            + 3 * context.spec.log_topic_gas
            + context.spec.log_data_gas * log_data_length
    };
    ...
    Ok(call_gas + transaction_gas + address_mapping_gas + call_log_gas + return_log_gas)
}
...
pub fn withdraw_gas(spec: &Spec) -> U256 {
    ...
    let log_gas = spec.log_gas + spec.log_topic_gas * 3 + spec.log_data_gas * H256::len_bytes() * 2;
...
impl Exec for PassResult {
    fn exec(
        ...
        if !static_flag {
            ReturnEvent::log(&(), &self.apply_state, &self.resume.params, context)
        ...
        if let Ok(ref data) = self.return_data {
            let length = data.len();
            let return_cost = U256::from((length + 31) / 32 * context.spec.memory_gas);
...
pub fn process_trap<T>(
    result: Result<ExecTrap, vm::Error>,
...
pub fn withdraw_from_evmcore(
    ...
    if balance < value {
        internal_bail!("Not enough balance for withdrawing from mapped address");
    }
    context.state.transfer_balance(
        &mapped_address,
...
pub fn mapped_balance(address: Address, context: &mut InternalRefContext) -> vm::Result<U256> {
    Ok(context.state.balance(&evm_map(address))?)
}

pub fn mapped_nonce(address: Address, context: &mut InternalRefContext) -> vm::Result<U256> {
...
pub fn build_bloom_and_recover_phantom(
    ...
    for log in logs.iter() {
        ...
        if log.address == *CROSS_SPACE_CONTRACT_ADDRESS {
            ...
            if event_sig == &CallEvent::EVENT_SIG || event_sig == &CreateEvent::EVENT_SIG {
                assert!(maybe_working_tx.is_none());

                let from = Address::from(Bytes20::abi_decode(&log.topics[1].as_ref()).unwrap());
                let to = Address::from(Bytes20::abi_decode(&log.topics[2].as_ref()).unwrap());
                ...
                let from = Address::from(Bytes20::abi_decode(&log.topics[1].as_ref()).unwrap());
                let (value, nonce) = ABIDecodable::abi_decode(&log.data).unwrap();
                ...
                let success: bool = ABIDecodable::abi_decode(&log.data).unwrap();

                let mut working_tx = std::mem::take(&mut maybe_working_tx).unwrap();
