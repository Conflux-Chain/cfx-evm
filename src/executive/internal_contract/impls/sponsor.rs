...
pub fn set_sponsor_for_gas(
    ...
    let (spec, state, substate): (&Spec, &mut dyn StateOpsTrait, &mut dyn SubstateTrait) =
    ...
    let prev_sponsor_balance = state.sponsor_balance_for_gas(&contract_address)?;
    ...
    if prev_sponsor.as_ref().map_or_else(
        ...
        // the upper bound.
        if prev_sponsor_balance >= prev_upper_bound && upper_bound < prev_upper_bound {
        ...
        if prev_sponsor.is_some() {
            tracer.trace_internal_transfer(
                ...
                AddressPocket::Balance(prev_sponsor.unwrap().with_native_space()),
        ...
        state.set_sponsor_for_gas(&contract_address, sponsor, &sponsor_balance, &upper_bound)?;
    } else {
...
pub fn set_sponsor_for_collateral(
    ...
    let prev_sponsor_balance = state.sponsor_balance_for_collateral(&contract_address)?;
    let collateral_for_storage = state.collateral_for_storage(&contract_address)?;
    // If previous sponsor is not the same as current sponsor, we should try
    // to replace the sponsor. Otherwise, we should try to charge
    // `sponsor_balance`.
    if prev_sponsor.as_ref().map_or_else(
...
pub fn add_privilege(
    ...
    for user_addr in addresses {
        state.add_commission_privilege(contract, params.storage_owner, user_addr)?;
    }
...
pub fn remove_privilege(
    ...
    for user_addr in addresses {
        state.remove_commission_privilege(contract, params.storage_owner, user_addr)?;
    }
    Ok(())
}
