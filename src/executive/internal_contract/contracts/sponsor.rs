    ...
    fn execute_inner(
        ...
        if !context.is_contract_address(&params.sender)? {
            return Err(vm::Error::InternalContract(
                "normal account is not allowed to set commission_privilege".into(),
            ));
        }
...
impl UpfrontPaymentTrait for AddPrivilegeByAdmin {
    fn upfront_gas_payment(
        &self,
        (_contract, addresses): &(Address, Vec<Address>),
        _: &ActionParams,
        context: &InternalRefContext,
    ) -> DbResult<U256> {
...
impl SimpleExecutionTrait for AddPrivilegeByAdmin {
    fn execute_inner(
        &self,
        (contract, addresses): (Address, Vec<Address>),
        params: &ActionParams,
        context: &mut InternalRefContext,
        _: &mut dyn VmObserve,
    ) -> vm::Result<()> {
