...
impl CommonParams {
    ...
    pub fn base_reward_in_ucfx(&self, _past_block_count: u64, height: BlockHeight) -> U512 {
        let (_, start_base_ward) = self
            .base_block_rewards
            .iter()
            .rev()
    ...
    pub fn custom_prefix(&self, height: BlockHeight) -> Option<Vec<Bytes>> {
        if height >= self.transition_heights.cip40 && height < self.transition_heights.cip94 {
