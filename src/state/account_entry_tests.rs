...
use cfx_storage::{tests::new_state_manager_for_unit_test, StorageManagerTrait};
use cfx_types::{address_util::AddressUtil, Address, AddressSpaceUtil, U256};
use primitives::{
    account::ContractAccount, storage::STORAGE_LAYOUT_REGULAR_V0, Account, SponsorInfo,
    VoteStakeList,
};
...
fn test_deposit_and_withdraw() {
    ...
    for _ in 0..100000 {
        ...
        accumulated_interest_rate.push(
            last * (*INITIAL_INTEREST_RATE_PER_BLOCK + *INTEREST_RATE_PER_BLOCK_SCALE)
...
fn check_ordered_feature(vote_stake_list: &VoteStakeList) {
    for i in 1..vote_stake_list.len() {
        assert!(
            vote_stake_list[i - 1].unlock_block_number < vote_stake_list[i].unlock_block_number
        );
...
fn init_test_account() -> OverlayAccount {
    ...
    let address_with_space = address.with_native_space();
    let account =
        Account::new_empty_with_balance(&address_with_space, &10_000_000.into(), &U256::zero());

    let mut overlay_account = OverlayAccount::from_loaded(&address_with_space, account.clone());
    overlay_account
        .cache_staking_info(
            true, /* cache_deposit_list */
            true, /* cache_vote_list */
...
fn test_vote_lock() {
    ...
    let mut overlay_account = init_test_account();
    overlay_account.vote_lock(U256::from(1000), 20 /* unlock_block_number */);
    check_ordered_feature(overlay_account.vote_stake_list().unwrap());
    assert_eq!(
        overlay_account.withdrawable_staking_balance(0 /* block_number */),
        U256::from(9900000)
    ...
    overlay_account.remove_expired_vote_stake_info(0 /* block_number */);
    assert_eq!(overlay_account.vote_stake_list().unwrap().len(), 4);
    overlay_account.vote_lock(U256::from(100000), 10 /* unlock_block_number */);
    check_ordered_feature(overlay_account.vote_stake_list().unwrap());
    assert_eq!(
        overlay_account.withdrawable_staking_balance(0 /* block_number */),
        U256::from(9900000)
    ...
    overlay_account.remove_expired_vote_stake_info(0 /* block_number */);
    assert_eq!(overlay_account.vote_stake_list().unwrap().len(), 4);
    overlay_account.vote_lock(U256::from(1000000), 11 /* unlock_block_number */);
    check_ordered_feature(overlay_account.vote_stake_list().unwrap());
    assert_eq!(
        overlay_account.withdrawable_staking_balance(0 /* block_number */),
        U256::from(9000000)
    ...
    overlay_account.vote_lock(U256::from(1000000), 13 /* unlock_block_number */);
    check_ordered_feature(overlay_account.vote_stake_list().unwrap());
    assert_eq!(
        overlay_account.withdrawable_staking_balance(0 /* block_number */),
        U256::from(9000000)
    ...
    overlay_account.vote_lock(U256::from(2000000), 40 /* unlock_block_number */);
    check_ordered_feature(overlay_account.vote_stake_list().unwrap());
    assert_eq!(
        overlay_account.withdrawable_staking_balance(0 /* block_number */),
        U256::from(8000000)
    ...
    overlay_account.vote_lock(U256::from(10), 600 /* unlock_block_number */);
    check_ordered_feature(overlay_account.vote_stake_list().unwrap());
    assert_eq!(
        overlay_account.withdrawable_staking_balance(0 /* block_number */),
        U256::from(8000000)
    ...
    overlay_account.vote_lock(U256::from(1000), 502 /* unlock_block_number */);
    check_ordered_feature(overlay_account.vote_stake_list().unwrap());
    assert_eq!(
        overlay_account.withdrawable_staking_balance(0 /* block_number */),
        U256::from(8000000)
    ...
    overlay_account.vote_lock(U256::from(3000000), 550 /* unlock_block_number */);
    check_ordered_feature(overlay_account.vote_stake_list().unwrap());
    assert_eq!(
        overlay_account.withdrawable_staking_balance(0 /* block_number */),
        U256::from(7000000)
...
fn test_clone_overwrite() {
    ...
    let mut overlay_account1 = OverlayAccount::from_loaded(&address_with_space, account1.clone());
    let mut overlay_account2 = OverlayAccount::from_loaded(&address_with_space, account2.clone());
