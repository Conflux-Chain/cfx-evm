...
use crate::{
    spec::genesis::DEV_GENESIS_KEY_PAIR, test_helpers::get_state_for_genesis_write, vm::Spec,
};
...
use cfx_storage::{
    tests::new_state_manager_for_unit_test, StateIndex, StorageManager, StorageManagerTrait,
};
...
#[cfg(test)]
fn get_state(storage_manager: &Arc<StorageManager>, epoch_id: &EpochId) -> State {
...
fn kill_account_with_checkpoints() {
    ...
    state_0.checkpoint();
    state_0
        .new_contract_with_code(&a_s, *COLLATERAL_DRIPS_PER_STORAGE_KEY, U256::one())
        .unwrap();
    state_0
    ...
    state_0
        ...
        .unwrap();
    state_0
        .add_sponsor_balance_for_collateral(&a, &COLLATERAL_DRIPS_PER_STORAGE_KEY)
        .unwrap();
    state_0
...
fn check_result_of_simple_payment_to_killed_account() {
    ...
    state_0
        ...
        .unwrap();
    state_0
        .add_collateral_for_storage(&sender_addr, &COLLATERAL_DRIPS_PER_STORAGE_KEY)
        .unwrap();
    state_0.discard_checkpoint();
    let epoch_id_1 = EpochId::from_uint(&U256::from(1));
    state_0
...
fn create_contract_fail_previous_storage() {
    ...
    substates.clear();
    substates.push(Substate::new());
    state = get_state(&storage_manager, &BigEndianHash::from_uint(&U256::from(1)));
    assert_eq!(
        state.total_storage_tokens(),
        *COLLATERAL_DRIPS_PER_STORAGE_KEY
    );
    ...
    drop(
        state
            .require_or_new_basic_account(&a_s, &Spec::new_spec_for_test().account_start_nonce)
            .unwrap(),
    );
    state
