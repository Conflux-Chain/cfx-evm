...
use primitives::{
    is_default::IsDefault, Account, CodeInfo, DepositInfo, DepositList, SponsorInfo, StorageKey,
...
pub struct OverlayAccount {
    ...
    // Sync.
    storage_owner_lv2_write_cache: RwLock<Arc<HashMap<Vec<u8>, Option<Address>>>>,
...
impl OverlayAccount {
    ...
    fn fresh_storage(&self) -> bool {
        let builtin_address =
            self.address.space == Space::Native && self.address.address.is_builtin_address();
    ...
    pub fn set_sponsor_for_collateral(&mut self, sponsor: &Address, sponsor_balance: &U256) {
        self.address.assert_native();
    ...
    pub fn check_commission_privilege(
        ...
        special_key.extend_from_slice(COMMISSION_PRIVILEGE_SPECIAL_KEY.as_bytes());
    ...
    pub fn withdrawable_staking_balance(&self, block_number: u64) -> U256 {
        ...
        return vote_stake_list.withdrawable_staking_balance(self.staking_balance, block_number);
    }
    ...
    #[cfg(test)]
    pub fn storage_owner_lv1_write_cache(&self) -> &HashMap<Vec<u8>, Option<Address>> {
    ...
    pub fn deposit(
        ...
        let not_maintain_deposit_list = cip_97 && self.deposit_list.as_ref().unwrap().0.is_empty();
    ...
    pub fn clone_dirty(&self) -> Self {
        ...
        account.storage_owner_lv1_write_cache = self.storage_owner_lv1_write_cache.clone();
    ...
    pub fn set_storage(&mut self, key: Vec<u8>, value: U256, owner: Address) {
        ...
        if self.address.space == Space::Ethereum || self.address.address == *SYSTEM_STORAGE_ADDRESS
        {
            return;
        }
        let lv1_write_cache = Arc::make_mut(&mut self.storage_owner_lv1_write_cache);
        if value.is_zero() {
    ...
    pub fn change_storage_value(&mut self, db: &StateDb, key: &[u8], value: U256) -> DbResult<()> {
        ...
        if !current_value.is_zero() {
            ...
            Arc::make_mut(&mut self.storage_value_write_cache).insert(key.to_vec(), value);
        } else {
    ...
    pub fn overwrite_with(&mut self, other: OverlayAccount) {
        ...
        self.storage_owner_lv2_write_cache = other.storage_owner_lv2_write_cache;
        self.storage_owner_lv1_write_cache = other.storage_owner_lv1_write_cache;
    ...
    pub fn original_ownership_at(&self, db: &StateDb, key: &Vec<u8>) -> DbResult<Option<Address>> {
        ...
        let storage_value_read_cache = &mut self.storage_value_read_cache.write();
        let storage_owner_lv2_write_cache = &mut *self.storage_owner_lv2_write_cache.write();
        let storage_owner_lv2_write_cache = Arc::make_mut(storage_owner_lv2_write_cache);
        Self::get_and_cache_storage(
        ...
        for (k, current_owner_opt) in storage_owner_lv1_write_cache {
            ...
            if original_ownership_opt != current_owner_opt {
                if let Some(original_owner) = original_ownership_opt.as_ref() {
                    ...
                    substate
                        .record_storage_release(original_owner, COLLATERAL_UNITS_PER_STORAGE_KEY);
                }
                if let Some(current_owner) = current_owner_opt.as_ref() {
                    // The owner has occupied a new key.
                    substate.record_storage_occupy(current_owner, COLLATERAL_UNITS_PER_STORAGE_KEY);
                }
            }
    ...
    pub fn commit(
        ...
        let storage_owner_lv2_write_cache = &**self.storage_owner_lv2_write_cache.read();
        ...
        if let Some(deposit_list) = self.deposit_list.as_ref() {
            self.address.assert_native();
            let storage_key = StorageKey::new_deposit_list_key(&self.address.address)
                .with_space(self.address.space);
            state
                .db
                .set::<DepositList>(storage_key, deposit_list, debug_record.as_deref_mut())?;
        }

        if let Some(vote_stake_list) = self.vote_stake_list.as_ref() {
            self.address.assert_native();
            let storage_key =
                StorageKey::new_vote_list_key(&self.address.address).with_space(self.address.space);
